...
export const useIndexModule = defineStore('module-index', {
  ...
  getters: {
    ...
    formatFloraAmount() {
      return (token: { amount: string; denom: string }) => {
        // Special handling for Flora's 18 decimal places
        if (token && token.amount) {
          const amount = Number(token.amount) / 1e18;
          if (amount >= 1e9) {
            return numeral(amount / 1e9).format('0,0.[00]') + 'B';
          } else if (amount >= 1e6) {
            return numeral(amount / 1e6).format('0,0.[00]') + 'M';
          } else {
    ...
    stats() {
      ...
      return [
        ...
        },
        {
          ...
          stats: this.formatFloraAmount()(bank.supply),
          change: 0,
        },
        {
          title: 'Bonded Tokens',
